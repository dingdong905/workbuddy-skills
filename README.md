...
WorkBuddy 与 Codex 各自维护完整、原生兼容的一套 Skill，不做跨平台文件级去重，也不使用软链接共享运行时目录。可以把另一套中经过验证的流程、reference、脚本思路和评测案例择优迁入，但必须保留本平台的 frontmatter、工具名、权限边界和安装方式。

因此同名或同源 Skill 可以同时存在并独立演化；“已迁移”表示能力经过本平台适配与验证，不表示两侧文件应逐行一致。
